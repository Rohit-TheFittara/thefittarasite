...
export default function Home() {
  ...
  const normalizedDigits = mobileNumber.replace(/\D/g, "");
  const isValid = normalizedDigits.length >= 10;
  ...
  function closePopup() {
    ...
    sessionStorage.setItem(numberKey, normalizedDigits);
    setShowPopup(false);
  }
    ...
    <main className="min-h-screen bg-transparent text-slate-900 flex flex-col">
      ...
      {showPopup ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm px-4">
          <div className="relative w-full max-w-md rounded-3xl bg-[#7c27f6] px-8 py-10 text-center text-white shadow-2xl">
            <button
              type="button"
              onClick={closePopup}
            ...
            <h2 className="text-2xl font-semibold mb-2">
              Please enter your watsapp number
            </h2>
            <p className="text-sm text-white/80 mb-6">
              In case we loose connection, we will reach out to you here.
            </p>
            <form
              className="space-y-4"
              onSubmit={(event) => {
            ...
            >
              <input
                ...
                className={`w-full rounded-full border bg-transparent px-4 py-3 text-sm text-white placeholder:text-white/70 focus:outline-none focus:ring-2 ${
                  touched && !isValid
                    ? "border-white/70 focus:ring-white/70"
                    : "border-white/60 focus:ring-white/80"
                }`}
              />
              <button
                type="submit"
                className="w-full rounded-full bg-white py-3 text-sm font-semibold text-[#7c27f6] hover:bg-white/90"
              >
                Submit
              </button>
              {touched && !isValid ? (
